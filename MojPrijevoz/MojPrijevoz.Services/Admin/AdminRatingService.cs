...
public class AdminRatingService : BaseAdminCrudService<Database.Rating, AdminRatingUpdateRequest,
    AdminRatingUpdateRequest, BaseRequestChanges, AdminRatingReponse, AdminAllRatingsResponse, AdminRatingSearchObject>
{
    ...
    public override async Task<IQueryable<Database.Rating>> ApplyFilter(IQueryable<Database.Rating> queryable,
        AdminRatingSearchObject searchObject)
    {
        ...
        if (!string.IsNullOrEmpty(searchObject.Contains))
            queryable = queryable.Where(it =>
                it.From!.User!.FirstName.ToLower().Contains(searchObject.Contains.ToLower())
    ...
    protected override async Task PrepareForResponse(Database.Rating entity, MojPrijevozDbContext dbContext)
    {
        ...
        entity.From = await _dbContext.UserProfiles.Include(it => it.User).Where(it => it.Id == entity.FromId)
            .FirstAsync();
        ...
        entity.Fare = await _dbContext.Fares.Include(it => it.FareData).ThenInclude(it => it!.OriginCity)
            .Where(it => it.Id == entity.FareId).FirstAsync();
    }
