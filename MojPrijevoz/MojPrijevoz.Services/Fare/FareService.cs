...
public class FareService :
    BaseCrudService<Database.Fare, FareInsertRequest, FareInsertRequest, FareResponse, FareSearchObject>, IFareService
{
    private readonly BaseFareState _baseFareState;
    private readonly INotificationService _notificationService;

    public FareService(MojPrijevozDbContext context, IMapper mapper, AuthorizationService authorizationService,
        BaseFareState baseFareState, INotificationService notificationService) : base(context, mapper,
    ...
    protected override IQueryable<Database.Fare> ApplyOrdering(IQueryable<Database.Fare> queryable,
        FareSearchObject searchObject)
    {
    ...
    public override async Task<IQueryable<Database.Fare>> ApplyFilter(IQueryable<Database.Fare> queryable,
        FareSearchObject searchObject)
    {
    ...
    protected override async Task PrepareForResponse(Database.Fare entity, MojPrijevozDbContext dbContext)
    {
        ...
        entity.FareData!.StopPoints =
            await _dbContext.StopPoints.Where(it => it.FareDataId == entity.FareDataId).ToListAsync();
        ...
        entity.Passenger = await _dbContext.UserProfiles.Include(it => it.User)
            .FirstAsync(it => it.Id == entity.PassengerId);
        entity.FareOffers = await _dbContext.FareOffers.Where(it => it.FareId == entity.Id).OrderBy(fo => fo.CreatedAt)
            .ToListAsync();
        entity.UserVehicle = await _dbContext.UserVehicles.Include(it => it.Vehicle)
            .FirstOrDefaultAsync(it => it.Id == entity.UserVehicleId);
    }
}