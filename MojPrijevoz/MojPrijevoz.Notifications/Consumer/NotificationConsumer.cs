...
public class NotificationConsumer : INotificationConsumer
{
    ...
    public async Task StartConsumingAsync()
    {
        ...
        await _bus.PubSub.SubscribeAsync<SubscribeToFcmDto>("subscribe-to-fcm-consumer",
            async request => await HandleSubscribeToFcmAsync(request));
        await _bus.PubSub.SubscribeAsync<UnSubscribeFromFcmDto>("unsubscribe-from-fcm-consumer",
            async request => await HandleUnSubscribeFromFcmAsync(request));
        await _bus.PubSub.SubscribeAsync<SendToUserDto>("send-to-user-consumer",
            async request => await HandleSendToUserAsync(request));
        await _bus.PubSub.SubscribeAsync<SendSilentToUserDto>("send-silent-to-user-consumer",
            async request => await HandleSilentSendToUserAsync(request));
    }
